        ...
        <div class="task-list-container">
            <div class="task-list-body" id="taskListBody">
                ...
                <div class="p-12 text-center">
                    <!-- Text Content -->
                    <h3 class="text-xl font-bold text-slate-800 mb-2">Belum Ada Tugas</h3>
                    <p class="text-slate-500 mb-6 max-w-md mx-auto">
                        Mulai produktivitasmu dengan membuat tugas pertama. Kelola dan pantau semua aktivitasmu di sini.
                    </p>

                    <!-- Action Button -->
                    <a href="{{ route('tasks.create') }}"
                        class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 
              ...
              shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105">
                    ...
                    <!-- Decorative Elements -->
                    <div class="mt-8 flex items-center justify-center space-x-2 text-slate-300">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @if(session('success'))
        ToastNotification.success('Berhasil!', '{{ session('
            success ') }}', 5000);
        @endif

        @if(session('error'))
        ToastNotification.error('Error!', '{{ session('
            error ') }}', 5000);
        @endif

        @if(session('warning'))
        ToastNotification.warning('Perhatian!', '{{ session('
            warning ') }}', 5000);
        @endif

        @if(session('info'))
        ToastNotification.info('Info', '{{ session('
            info ') }}', 5000);
        @endif
    });
